...
it('EP-B22 a delayed search URL update preserves characters typed since submission', () => {
  ...
  const view = render(<EntitySearch label="Search" query="" navigate={navigate} />);
  ...
  view.rerender(<EntitySearch label="Search" query="Note" navigate={navigate} />);
...
it('EP-B15 clearing a committed search does not resurrect the original draft', () => {
  ...
  view.rerender(<EntitySearch label="Search" query="Notes" navigate={navigate} />);
  view.rerender(<EntitySearch label="Search" query="" navigate={navigate} />);
  navigate.mockClear();
...
it('EP-B15 resetting search cancels a draft before its debounce finishes', () => {
  vi.useFakeTimers();
  const navigate = vi.fn();
  const view = render(<EntitySearch key={0} label="Search" query="" navigate={navigate} />);
  fireEvent.change(screen.getByRole('textbox'), { target: { value: 'Notes' } });
  view.rerender(<EntitySearch key={1} label="Search" query="" navigate={navigate} />);
  act(() => vi.advanceTimersByTime(1000));
  expect(screen.getByRole('textbox')).toHaveProperty('value', '');
  expect(navigate).not.toHaveBeenCalled();
});
