...
const ops = {
  ...
  update: (
    ...
    entityId: string,
    revision: number,
    fields: Parameters<typeof repo.updateCommittee>[3],
  ) => repo.updateCommittee(ctx.db, entityId, revision, fields, ctx.user.id),
  restore: (ctx: Context, entityId: string, opId: string) =>
    repo.restoreCommittee(ctx.db, entityId, opId, ctx.user.id),
};
export async function listCommittees(ctx: Context, query: CommitteeListQuery) {
  ...
  const rows = await repo.selectCommittees(
    ctx.db,
    query,
    date,
    spec,
  ...
  return {
    data: page.map((row) => Committee.parse(row)),
    meta: {
      counts: await repo.committeeCounts(ctx.db, query, date),
      taskStats: await repo.taskStats(ctx.db, query, date),
...
export async function patchCommittee(ctx: Context, committeeId: string, input: CommitteePatch) {
  ...
  void revision;
  await applyUpdate(ctx, ops, row, fields, {
...
export async function requireCommittee(
  ...
  if (row.status !== 'active')
    throw new AppError('rule_violation', {
      rule: 'COMM-B03',
      fieldErrors: { committeeId: ['archived'] },
    });
}
...
export async function activity(ctx: Context, committeeId: string, cursor?: string) {
  ...
  const rows = await repo.selectActivity(
    ctx.db,
    committeeId,
    decodeCursor(cursor, repo.activitySort, hash),
  );
  ...
  return {
    data: page,
    meta: {
      nextCursor: rows.length > 50 && last ? encodeCursor(repo.activitySort, hash, last) : null,
    },
  };
}
