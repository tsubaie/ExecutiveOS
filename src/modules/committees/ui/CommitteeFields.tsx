...
export function CommitteeFields({ initial, save, nameError }: { initial: CommitteeCreate; save?: (patch: Omit<CommitteePatch, 'revision'>) => void; nameError?: string | undefined }) {
  ...
  return <fieldset data-autosave={save ? true : undefined} className="grid gap-4">
    <Field label={t('name')} error={nameError} quiet={Boolean(save)}>{(control) => <Input {...control} name="name" dir="auto" required pattern=".*\S.*" maxLength={500}
      defaultValue={initial.name} onBlur={(event) => { const name = event.target.value.trim(); if (name && name !== initial.name) save?.({ name }); }} />}</Field>
    <Property label={t('scope')} quiet={Boolean(save)}><ChoiceSelect label={t('scope')} name="scope" value={properties.scope}
    ...
    <Property label={t('status')} quiet={Boolean(save)}><ChoiceSelect label={t('status')} name="status" value={properties.status}
    ...
    <Field label={t('ownership')} quiet={Boolean(save)} empty={!initial.ownership}>{(control) => <Input {...control} name="ownership" dir="auto" maxLength={500} defaultValue={initial.ownership}
      onBlur={(event) => { if (event.target.value.trim() !== initial.ownership) save?.({ ownership: event.target.value.trim() }); }} />}</Field>
    <Field label={t('description')} quiet={Boolean(save)} empty={!initial.description}>{(control) => <Textarea {...control} name="description" dir="auto" rows={4} maxLength={50000} defaultValue={initial.description}
