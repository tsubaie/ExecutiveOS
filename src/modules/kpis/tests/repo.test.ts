...
import { quarterOf, quarterIndex, quarterRange, previousQuarter } from '@/core/time/kpis';
...
async function window() {
  ...
  const before = previousQuarter(quarterOf(today));
  const range = quarterRange(before);
  return {
    today,
    earliest: quarterIndex(before),
    previousFrom: range.from,
    previousTo: range.to,
  };
}
const empty = { view: 'all', q: '', objectiveId: '', category: '', team: '' };
...
it('KPIS-B07 the candidate select carries deleted rows so every view counts from one result', async () => {
  ...
  const rows = await repo.selectKpis(db(), empty, await window());
...
it('KPIS-B05 the candidate select reads the current, previous and sparkline values as numbers', async () => {
  ...
  const [row] = await repo.selectKpis(db(), empty, await window());
...
it('KPIS-B02 only the quarters a status or a comparison can reach are fetched with the row', async () => {
  const today = await workspaceToday();
  const current = quarterOf(today);
  const before = previousQuarter(current);
  const kpi = await harness.kpi('Windowed');
  await harness.targets(kpi.id, [
    { year: before.year - 2, quarter: 1, targetValue: 1 },
    { ...before, targetValue: 2 },
    { ...current, targetValue: 3 },
  ]);
  const [row] = await repo.selectKpis(db(), empty, await window());
...
it('KPIS-B07 a search term matches the KPI text and excludes the rest', async () => {
  ...
  const rows = await repo.selectKpis(db(), { ...empty, q: 'promoter' }, await window());
  expect(rows.map((row) => row.id)).toEqual([found.id]);
  expect(await repo.selectKpis(db(), { ...empty, q: 'Service' }, await window())).toHaveLength(1);
});
