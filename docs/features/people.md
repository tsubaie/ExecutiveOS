...
- PEOPLE-B02 **List**: rows with avatar (initials; Arabic names use the first letter of the first two words), name, organization and role, kind chip, open task count (owner edges to non-completed tasks), next meeting date (attendee edges, `starts_at ≥ now`). Views `all` (default), `assignable`, `internal`, `external`, `trash`. Facets: tag, organization, has-open-tasks. Search: name, display name, organization, role, email (the list's field is labelled `people.searchList`, EP-B41). Sort: name (default), open tasks desc, next meeting asc.
...
- PEOPLE-B07 **Delete**: deletion runs on the press through the shared entity behaviour (EP-B36) with no confirmation, because trash is reversible. The panel closes and a toast reads "Person moved to trash" with Undo, which restores the person through the deletion's operation id. A failed delete leaves the panel open and the person untouched, with the error above it.
...
- ui: avatar initials en/ar, duplicate confirmation, tabs, undoable deletion (B07), avatar name reveal by pointer and keyboard (B09).
