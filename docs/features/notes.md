...
- NOTES-B22 **AI acts on fields, not on the record.** Refining sits on the Content label row and tag suggestion on the Tags label row, each beside what it changes, not in a band above the record's title: they transform one field, a reader reaches for them while looking at that field, and at the top they took the first read from the note's own name. Both are one icon: the label beside them supplies the noun, and a text button on the row pulled the eye from the reader's own prose; refining names itself on hover and for assistive technology, tag suggestion likewise, and its action is additive and reversible — the reader ticks which tags to keep — and the whole exchange happens in a popover hanging off that icon so the field never leaves their sight. Refining is hidden while the note is empty rather than shown disabled: an action that cannot apply yet is not information. Where a capability is unavailable, its notice takes the same slot (ADMIN-B29).
...
- NOTES-B28 **Expanded view for the content.** The Content label row carries an Expand control (and, once open, Exit expanded view in the record's place). It moves the same field, preview and textarea alike with everything it can do (B07, B08, B23–B25), into a dialog that takes most of the window, headed by the note's title and holding nothing else, with the text set at a reading measure and looser leading (EP-B44). The state travels in the URL as `focus=content`, so a refresh or a shared link opens the same view, and it ends when the record or the list changes. Escape from the textarea leaves the text first, the next Escape closes the view, the one after closes the record; an open mention list keeps its own Escape. While the text is being written there, a toolbar pinned above it offers Title (`##`), Subtitle (`###`), Bold, Italic, Bullet list and Checklist: the inline actions act on the selection like their keys (B23); the heading and list actions mark every selected line or unmark them all when every line already carries that marker; a press keeps focus in the text; targets are 40 px, 44 px under a coarse pointer, and carry their name beside the icon when there is room. The view uses its room: the writing column is wide and the text fills the height.
...
- NOTES-B13 **Tags** `GET /notes/tags` returns distinct tags over non-deleted notes with counts, sorted by count then name; used for the facet and the editor's picker. Suggestions include only tags still used by at least one non-deleted note. The editor refreshes suggestions on focus, disables browser form-history suggestions, and excludes tags already selected case-insensitively. The picker is a listbox under the input (`role=combobox` with `aria-activedescendant`): typing filters the suggestions; Arrow keys move, Enter picks, and a click or tap on a row adds that tag at once with no Enter. A typed name that matches no suggestion is offered as an "Add" row and is also added with Enter, comma, or leaving the field.
...
- NOTES-B18 **Refinement review (Mission Control parity)**: A note with content has a Refine action when AI is available, with refining progress and retry on failure. A completed proposal never replaces the record: it waits as a strip on the Content field, "A suggested rewrite is ready" with Review; while it works the label row reads Refining… with the seconds after a moment and Cancel, and after a failure it reads Couldn't refine with the reason on hover and Retry; a refinement that could not start says why in one muted sentence on the field; cancellation and application leave no notice behind beyond their toasts; the unavailable notice keeps the label row (ADMIN-B29); Review opens the expanded view (B28) as a review surface. Its header offers a view switch, Suggested rewrite (the default, editable in the field itself), Original (the text captured from the job input, read-only, in the rewrite's place) and, from 1024 px, Side by side; a status line appears only when the note changed since the rewrite; and one line "What changed" carries the model's summary, opening on More when it is longer. Beside the text from 1024 px, and below it on narrower screens, an "Also create" card lists the suggested tasks as rows with a checkbox, the title editable in place (1–120 trimmed characters), status and priority chips, and the source sentence behind a Source toggle; one master checkbox in the card's heading ticks or unticks them all, with an indeterminate state in between; the suggested tags sit at the card's foot as small chips, under a rule. Tasks and tags start selected. A footer pinned to the view's bottom holds Discard at the start and one action at the end whose name says what it does: Use rewrite, or Use rewrite and create N tasks (استخدام المقترح). Keep applies the rewrite as the field shows it (an edited draft travels as `content`, accepted only with `acceptContent`) together with the selected tags, and creates the selected tasks using reviewed titles; unticking the tasks keeps the rewrite alone. Closing the expanded view (Escape or its control) keeps the pending proposal for reopening. Discard dismisses the proposal and the toast offers Undo, which restores it through `POST /notes/:id/refine/restore` as pending, or stale when the note moved on, until it is applied. Stale proposals cannot be kept: the status says the note moved on, the card is absent, and the footer offers regeneration beside Discard. All writes retain B17's transactional revision and repeat-apply guarantees. Prompt v2 mirrors Mission Control's structure guidance: meeting/planning/general sections as appropriate, minimal changes for short or already structured notes, no invented facts, existing-tag reuse. Mixed Arabic/English notes refine into Arabic while preserving names, technical terms and mentions; task titles retain their source passage's language. Existing prompt-v1 jobs remain executable. Development awaits tests/audits.
...
- NOTES-A14 Expand on a note's content opens the expanded view with the content and `focus=content` in the URL; Escape closes it and the record stays open. (en)
...
- service: I02 (disabled type on create and change), B18 edited keep and discard/restore, B27 (built-in templates, configured and disabled ones), I04 (duplicate participant, deleted person hidden), I06, I07; B01 defaults; B07 participant replacement; B12 four cases (plain, stale revision, tag overflow, already archived or tagged).
...
- ui: row, bands, detail edits, mention menu (filter, arrows, insert, dismissal, quick-create), derived participants, tag overflow block, tasks panel create/attach/detach/complete, editor enter and leave, writing keys (B23), timed commit and own round trip (B24), checklist toggle (B25), template select and locale fallback (B27), expanded view (B28), review surface and review mode (B18), note types editor.
- e2e `notes.spec.ts`: A01–A14 in the listed locales.
