...
- NOTES-B18 **Refinement review (Mission Control parity)**: A note with content has a Refine action when AI is available, with refining progress and retry on failure. A completed proposal never replaces the record: the Content label row's Review opens the expanded view (B28) with the rewrite in the field itself, editable and full height, under a slim band that names it and gives the change summary. Beside the text from 1024 px, and below it on narrower screens, a rail holds the suggested tags, the suggested task cards (status, priority, owner, due date, source snippet) and the original captured from the job input under a disclosure; tasks and tags start selected, task titles are editable (1–120 trimmed characters) with select/deselect-all controls. A footer pinned to the view's bottom holds Discard at the start and one Keep at the end whose name says what it does: Keep rewrite, or Keep with N tasks. Keep applies the rewrite as the field shows it (an edited draft travels as `content`, accepted only with `acceptContent`) together with the selected tags, and creates the selected tasks using reviewed titles; unticking the tasks keeps the rewrite alone. Closing the expanded view (Escape or its control) keeps the pending proposal for reopening. Discard dismisses the proposal and the toast offers Undo, which restores it through `POST /notes/:id/refine/restore` as pending, or stale when the note moved on, until it is applied. Stale proposals cannot be kept: the band says the note moved on, the rail hides the tick lists, and the footer offers regeneration beside Discard. All writes retain B17's transactional revision and repeat-apply guarantees. Prompt v2 mirrors Mission Control's structure guidance: meeting/planning/general sections as appropriate, minimal changes for short or already structured notes, no invented facts, existing-tag reuse. Mixed Arabic/English notes refine into Arabic while preserving names, technical terms and mentions; task titles retain their source passage's language. Existing prompt-v1 jobs remain executable. Development awaits tests/audits.
...
- ui: row, bands, detail edits, mention menu (filter, arrows, insert, dismissal, quick-create), derived participants, tag overflow block, tasks panel create/attach/detach/complete, editor enter and leave, writing keys (B23), timed commit and own round trip (B24), checklist toggle (B25), template select and locale fallback (B27), expanded view (B28), review band and review mode (B18), note types editor.
