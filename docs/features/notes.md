...
- NOTES-B18 **Refinement review (Mission Control parity)**: A note with content has a Refine action when AI is available, with refining progress and retry on failure. A completed proposal never replaces the record: the Content label row's Review opens the expanded view (B28) with the rewrite in the field itself, editable, under a proposal band that names it, gives the change summary, and offers Keep, Keep without tasks when tasks are suggested, Discard, and Show original, which places the original captured from the job input beside the rewrite from 1024 px and below it on narrower screens. Suggested tags and task cards (status, priority, owner, due date, source snippet) sit under the band; tasks and tags start selected, task titles are editable (1–120 trimmed characters) with select/deselect-all controls. Keep applies the rewrite as the field shows it (an edited draft travels as `content`, accepted only with `acceptContent`) together with the selected tags, and creates the selected tasks using reviewed titles unless Keep without tasks was chosen. Closing the expanded view (Escape or its control) keeps the pending proposal for reopening. Discard dismisses the proposal and the toast offers Undo, which restores it through `POST /notes/:id/refine/restore` as pending, or stale when the note moved on, until it is applied. Stale proposals cannot be kept and offer regeneration. All writes retain B17's transactional revision and repeat-apply guarantees. Prompt v2 mirrors Mission Control's structure guidance: meeting/planning/general sections as appropriate, minimal changes for short or already structured notes, no invented facts, existing-tag reuse. Mixed Arabic/English notes refine into Arabic while preserving names, technical terms and mentions; task titles retain their source passage's language. Existing prompt-v1 jobs remain executable. Development awaits tests/audits.
