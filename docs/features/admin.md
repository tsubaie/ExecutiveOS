...
- ADMIN-B30 Note templates editor (`notes.templates`) on `/admin/notes`: edit each template's label and body per locale, disable, add a template by identifier; an empty setting shows the built-in templates as the starting point (`NOTES-B27`).
...
- ADMIN-B29 `GET /ai/availability` answers three things, not one: the capabilities this reader can use, whether the workspace is `configured` for AI at all (a credential exists and at least one capability is switched on), and whether this reader `canConfigure` it (an administrator). A record shows the absence accordingly, in the slot and weight of the control it stands in for rather than above the record's own title. A workspace that has never set AI up is not faulty: only a reader who can turn it on is told, and they are invited to rather than asked to re-check a setting nobody changed. A workspace that does use AI and cannot reach it is told to everyone, with a re-check, because a member has lost a control they had and the answer can change. Either way one info control discloses what AI would do on that record, and the route to switch it on appears only for a reader whose role can, so nobody is sent to a page that refuses them. Role stays out of Account's schemas (ACCT-I02); this is AI answering for itself.
...
- ADMIN-B13 Restore is a command (`pnpm backup:restore <file>`) documented on the page; it verifies the backup, puts the app in maintenance mode, restores DB and files (ADMIN-B31), and exits maintenance. Maintenance mode returns 503 for all routes except health.
- ADMIN-B31 **Restore is failure-atomic for files.** `files.tar` is extracted into a unique staging directory inside `FILES_DIR` (never into live files), `pg_restore` runs in one transaction, and only after it commits are the live entries retired and the staged entries moved into place; a failed move puts both sets back, and if even that fails the retired entries stay in a `.previous-*` directory rather than being deleted. The staging directory is always removed and maintenance mode is always cleared, whether the restore succeeds or fails, and clearing it tolerates a file that is already gone. A stale staging directory left by an interrupted restore is removed at the start of the next one. Backups never capture restore staging or retired directories (ADR 0024).
- ADMIN-B32 **Restore refuses untrustworthy archives.** Before activation each file's size is compared with the manifest's `bytes` and then its checksum. Extraction streams through counters and stops at the first entry past a limit, before writing it: at most 200 000 entries, and expanded bytes no larger than both `FILES_QUOTA_GB` and the archive's own verified size. Only regular files and directories are accepted; symbolic links, hard links, device entries, `..` paths, absolute paths and truncated entries fail the restore instead of being skipped.
- ADMIN-B33 **Backups, restores and jobs stop cleanly.** Every phase of a backup (dump, archive, checksums, manifest, verification) observes the job's abort signal, and a backup that does not finish removes its staging directory. A restore observes `SIGINT`/`SIGTERM` and a one-hour deadline in every phase before the file swap; the swap itself is never abandoned half way because the database has already committed. On `SIGTERM` or `SIGINT` the job runner stops claiming, lets running handlers finish for up to `JOBS_DRAIN_SECONDS`, then aborts the remainder and gives them five seconds to record the attempt, which is retried later. The production image sets `NEXT_MANUAL_SIG_HANDLE` so the process exits after this drain (143 for `SIGTERM`, 130 for `SIGINT`) instead of when Next's HTTP server closes.
...
- core adversarial: backup during writes; restore drill; B31 failure at each restore stage and swap rollback; B32 traversal, absolute path, link, truncated entry, entry-count and quota fixtures and a manifest size mismatch; B33 aborted backup, restore deadline and interruption, graceful and forced runner drain.
