...
// Runs after the web server is up: empties a `_test` database (scripts/db/e2e-reset.ts), then
// provisions the browser-test administrator through the real first-run setup API when the
// workspace is empty, otherwise reuses the credentials file.
export default async function globalSetup(config: FullConfig) {
  const baseURL = config.projects[0]?.use.baseURL ?? 'http://localhost:3000';
  execFileSync(
    process.execPath,
    ['--conditions=react-server', '--import', 'tsx', 'scripts/db/e2e-reset.ts'],
