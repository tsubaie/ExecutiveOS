...
export const serverMarker = {
  ...
  create(context) {
    ...
    if (
      testFile(path) ||
      !/(?:\/src\/modules\/[^/]+\/(?:service|repo)\.ts$|\/schema\/db\.ts$|\/src\/core\/(?:db|auth|ai|jobs|files|backup|entity)\/|\/src\/core\/modules\/(?:registry|server-manifest)\.ts$|\/src\/core\/config\/env\.ts$)/u.test(
        path,
      )
    )
      return {};
